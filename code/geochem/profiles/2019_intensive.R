...
geochem.data <- read.csv("dataEdited/waterChemistry/geochem_WC_2019.csv",
...
Hg.data <- read.csv("dataEdited/waterChemistry/Hg_WC_2019_intensive.csv",
                    stringsAsFactors = FALSE)
...
for (RM.of.interest in site.list) {
  ...
  if (RM.of.interest %in% geochem.data$RM) {
    ...
    geochem.data.site <- geochem.data %>%
      ...
      select(c(depth, SO4_mg.L, NO3_mgN.L, NO2_mgN.L)) %>%
      as.data.frame()
