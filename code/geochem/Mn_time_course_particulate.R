...
inflow.data <- read_xlsx("dataRaw/dataRelease_chem/v2/HCC - V2 Data Release_Master File_Oct 2019 to present_V2_9_forModelingGroup.xlsx",
  ...
  mutate(date = date(ymd_hms(sample_collection_date_mm_dd_yy_h_mm)),
         p_mn_mg_per_l = as.numeric(p_mn_mcg_per_l) / 1000) %>%
  ...
  select(RM, date, depth, elevation_m, p_mn_mg_per_l) %>%
...
Mn.vector = colorize.function(7)
names(Mn.vector) <- floor_date(seq(from = as.Date("2017-05-01"),
...
function.of.function <- function(year.you.want) {
  time.course.profile.plot(geochem.data.to.use = geochem.data.of.interest,
                           ...
                           concentrations.to.use = c(0, 0.4),
                           RMs.to.use = c(286,300, 310),
                           parameter.to.plot = "p_mn_mg_per_l",
                           xlabel.to.use = "Particulate Mn (mg/L)",
                           color.vector.to.use = Mn.vector,
...
#### Save out Mn plots ####
pdf("results/geochem/Mn_time_course_particulate.pdf",
    width = 7,
    height = 8)
