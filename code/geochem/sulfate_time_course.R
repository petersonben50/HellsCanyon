#### code/geochem/sulfate_time_course.R ####
# Benjamin D. Peterson
...
inflow.data <- read_xlsx("dataRaw/dataRelease_chem/v2/HCC - V2 Data Release_Master File_Oct 2019 to present_V2_9_forModelingGroup.xlsx",
  ...
  select(RM, date, depth, elevation_m, f_so4_mg_per_l) %>%
...
sulfate.vector = colorize.function(7)
names(sulfate.vector) <- floor_date(seq(from = as.Date("2017-05-01"),
...
#### Function to generate sulfate plots ####
function.of.function <- function(year.you.want) {
  time.course.profile.plot(geochem.data.to.use = geochem.data.of.interest,
                           ...
                           concentrations.to.use = c(0, 60),
                           RMs.to.use = c(286,300, 310),
                           parameter.to.plot = "f_so4_mg_per_l",
                           xlabel.to.use = "Sulfate (mg/L)",
                           color.vector.to.use = sulfate.vector,
...
#### Save out sulfate plots ####
pdf("results/geochem/sulfate_time_course.pdf",
    width = 7,
    height = 8)
