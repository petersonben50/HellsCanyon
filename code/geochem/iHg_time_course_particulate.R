...
#### Function to generate particulate iHg plots ####
function.of.function <- function(year.you.want) {
  time.course.profile.plot(geochem.data.to.use = geochem.data.of.interest,
                           ...
                           concentrations.to.use = c(-0.1, 4.7),
                           RMs.to.use = c(286,300, 310),
                           parameter.to.plot = "iHg_part_ngL",
                           xlabel.to.use = "Particulate Hg(II)i (ng/L)",
...
#### Save out particulate Hg(II)i plots ####
pdf("results/geochem/iHg_time_course_particulate.pdf",
    width = 7,
    height = 8)
